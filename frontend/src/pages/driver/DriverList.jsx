...
const DriverList = () => {
  ...
  const fetchDrivers = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        "http://localhost:8000/api/driverList?time=" + new Date().getTime(),
        {
          headers: {
  ...
  const currentDrivers = filteredDrivers.slice(firstIndex, lastIndex);
  const totalPages = Math.ceil(filteredDrivers.length / itemsPerPage);

  const deleteDriver = async (_id, firstName, lastName) => {
    const token = localStorage.getItem("token");
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await axios.post(
            "http://localhost:8000/api/deleteDriver",
            { _id },
            {
              headers: {
                Authorization: `Bearer ${token}`,
          ...
          setDrivers(drivers.filter((u) => u._id !== _id));
  ...
  const handleToggle = async (id, currentStatus) => {
    ...
    try {
      const res = await axios.post(
        "http://localhost:8000/api/toggleStatusDriver",
        { id, status: newStatus }
      );
  ...
  if (loading) return <h3>Loading Drivers...</h3>;
                ...
                {currentDrivers.length === 0 ? (
                  ...
                  currentDrivers.map((driver, index) => (
                    <tr key={driver._id}>
                      ...
                      <td className="align-middle">{driver.phoneNumber}</td>
                      <td className="align-middle">
                        <i
                          ...
                          onClick={() =>
                            handleToggle(driver._id, driver.status)
                          }
                        ></i>
                      </td>
                      <td className="align-middle">
